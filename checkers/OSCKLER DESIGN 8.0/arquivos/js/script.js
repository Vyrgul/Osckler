	...
	function unique(array){ //função para remover linhas duplicadas
		return array.filter(function (el, index, arr){
			return index == arr.indexOf(el);
		});
	}

	function clean(str){ //função para remover espaços e tabs
	...
	function removerr() {
		...
		array = unique(array); //remove linhas duplicadas
		for(i=0;i<array.length;i++){
			array[i] = clean(array[i]); //remove espaços e tabs
		}
		array = array.filter(function(n){return n!=0});  //remove linhas vazias
...
function apertarbotaoparar() {
	document.getElementById("#parar").style.border = "4px double cyan";
}
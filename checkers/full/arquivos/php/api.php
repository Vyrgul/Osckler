...
if(isset($_GET['cc']) AND isset($_GET['deli']) AND strrpos($_GET['cc'], $_GET['deli']) AND (count($cc = explode($_GET['deli'], $_GET['cc'])) - 1) == 3 ){
	foreach($cc as $i): (empty($i) ? exit(
		...
		json_encode(array('status'=>3,'str'=>'<tr><td>'.$cc[0].'</td><td>'.$cc[1].'</td><td>'.$cc[2].'</td><td>'.$cc[3].'</td><td>INVALID</td></tr>', 'nsaldo' => $nsaldo))) : null); endforeach;
...
// REQUEST 1 //
                 ...
                 ((strrpos($result1, 'You must enter a valid credit card number')) ? exit(
			...
			if($_SESSION['admin'] == 0){
				$nsaldo = $nsaldo+1;
   				...
   				$nsaldo = null;
                 	...
                 	json_encode(array('status'=>3,'str'=>'<tr><td>'.$cc[0].'</td><td>'.$cc[1].'</td><td>'.$cc[2].'</td><td>'.$cc[3].'</td><td>INVALID CARD</td></tr>', 'nsaldo' => $nsaldo))) : NULL);
                 //'{"resultado":"DIE","retorno":"<span class=\"label btn_6 label-danger\">#DIE ✘</span> <span class=\"label label-warning\">'.$_GET['cc'].' INVALID</span>"}'
                 ((strrpos($result1, 'DebitCard is not accepted. Please try another credit card')) ? exit(
			...
			if($_SESSION['admin'] == 0){
				$nsaldo = $nsaldo+1;
   				...
   				$nsaldo = null;
                 	...
                 	json_encode(array('status'=>3,'str'=>'<tr><td>'.$cc[0].'</td><td>'.$cc[1].'</td><td>'.$cc[2].'</td><td>'.$cc[3].'</td><td>DEBIT CARD NOT ACCEPTED</td></tr>', 'nsaldo' => $nsaldo))) : NULL);
...
// REQUEST 2 //
			...
			if($_SESSION['admin'] == 0){
				mysqli_query($conecta,"UPDATE usuarios SET saldo = $nsaldo WHERE `usuario` = '".$_SESSION['usuario']."'");
			}else{
   				$nsaldo = null;
   			}

                        		json_encode(array('status'=>1,'str'=>'<tr><td>'.$cc[0].'</td><td>'.$cc[1].'</td><td>'.$cc[2].'</td><td>'.$cc[3].'</td><td>R$ '.$saldo.'</td><td>'.$bin->country->name.'</td><td>'.$bin->country->currency.'</td><td>'.$bin->bank->name.'</td><td>'.strtoupper($bin->scheme).'</td><td>'.strtoupper($bin->brand).'</td><td>'.strtoupper($bin->type).'</td></tr>', 'nsaldo' => $nsaldo)));
			...
			if($_SESSION['admin'] == 0){
				$nsaldo = $nsaldo+1;
   				...
   				$nsaldo = null;
                        		...
                        		json_encode(array('status'=>0,'str'=>'<tr><td>'.$cc[0].'</td><td>'.$cc[1].'</td><td>'.$cc[2].'</td><td>'.$cc[3].'</td><td>R$ '.$saldo.'</td></tr>', 'nsaldo' => $nsaldo)));
			...
			if($_SESSION['admin'] == 0){
				$nsaldo = $nsaldo+1;
   				...
   				$nsaldo = null;
                        		...
                        		json_encode(array('status'=>0,'str'=>'<tr><td>'.$cc[0].'</td><td>'.$cc[1].'</td><td>'.$cc[2].'</td><td>'.$cc[3].'</td><td>INVALID DATE CARD</td></tr>', 'nsaldo' => $nsaldo)));
                        	//'{"resultado":"DIE","retorno":"<span class=\"label btn_6 label-danger\">#DIE ✘</span> <span class=\"label label-warning\">'.$_GET['cc'].'</span>"}'
                        else:
                        	exit(
			...
			if($_SESSION['admin'] == 0){
				$nsaldo = $nsaldo+1;
   				...
   				$nsaldo = null;
                        		...
                        		json_encode(array('status'=>3,'str'=>'<tr><td>'.$cc[0].'</td><td>'.$cc[1].'</td><td>'.$cc[2].'</td><td>'.$cc[3].'</td><td>N/D</td></tr>', 'nsaldo' => $nsaldo)));
			...
			if($_SESSION['admin'] == 0){
				$nsaldo = $nsaldo+1;
   				...
   				$nsaldo = null;
                    		...
                    		json_encode(array('status'=>3,'str'=>'<tr><td>'.$cc[0].'</td><td>'.$cc[1].'</td><td>'.$cc[2].'</td><td>'.$cc[3].'</td><td>PARAMETROS NÂO INSERIDOS</td></tr>', 'nsaldo' => $nsaldo)));
                    }